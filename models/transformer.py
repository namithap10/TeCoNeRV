...
class Attention(nn.Module):
    ...
    def forward(self, fr, to=None):
        ...
        q, k, v = map(lambda t: einops.rearrange(
            t, 'b n (h d) -> b h n d', h=self.n_head), [q, k, v])
...
class PreNormXAttn(nn.Module):
    ...
    def forward(self, x, v):
        return self.fn(self.norm(x), self.v_norm(v))
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, dim, depth, n_head, head_dim, ff_dim, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim, n_head, head_dim, dropout=dropout)),
                PreNormXAttn(dim, Attention(
                    dim, n_head, head_dim, dropout=dropout)),
                PreNorm(dim, FeedForward(dim, ff_dim, dropout=dropout)),
            ]))

    def forward(self, x, v):
        for norm_attn, norm_xattn, norm_ff in self.layers:
            x = x + norm_attn(x)
            x = x + norm_xattn(x, v)
            x = x + norm_ff(x)
        return x
