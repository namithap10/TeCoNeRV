...
class HyperNeRV(nn.Module):
    ...
    def __init__(self, tokenizer, hyponet, n_tokens, token_dims, transformer_encoder):
        ...
        print(f'hyponet param shapes: {self.hyponet.param_shapes}')
    ...
    def from_checkpoint(cls, ckpt, load_state_dict=True):
        ...
        model = cls(tokenizer, hyponet, n_tokens, token_dims, transformer_encoder)
        if load_state_dict:
        ...
        for name, shape in self.hyponet.param_shapes.items():
            ...
            if r - l > 0:
                ...
                if quant_bit < 32: # Quantization aware training, disabled by default
                    x_quant_v, x_scale, x_t_min = quantize_per_tensor(x, bit=quant_bit, axis=1, dither=False)
                    x = x_t_min.to(torch.float32) + (x_scale.to(torch.float32) * x_quant_v)
                    quant_overhead_bits += ((x_scale.numel() + x_t_min.numel()) * 32 / x.shape[0])
                
                x = x.transpose(-1, -2) # (B, g, in_ch)
                pre_mod[name] = x.clone()
                repeat_num = init_w.nelement() // x.nelement()
                x = einops.repeat(x, 'B n m -> B n d m', d=repeat_num).reshape_as(init_w) # (B, in_ch, out_ch)
